...
public class FixGridLayout extends LinearLayout {
	...
	protected void onMeasure(int widthMeasureSpec, int heightMeasureSpec) {
		...
		for (int i = 0; i < mCount; i++) {
			...
			//+18是getPosition方法中marginleft+18
			int childw = child.getMeasuredWidth() + 18;
			...
			mBottom = mTop + child.getMeasuredHeight()+10;
			mY = mTop; // 每次的高度必须记录 否则控件会叠加到一起
			position.left = mLeft;
			position.top = mTop + 3;
			position.right = mRight - 1;
	...
	public int getPosition(int IndexInRow, int childIndex) {
		if (IndexInRow > 0) {
			...
			return getPosition(IndexInRow - 1, childIndex - 1)
					+ getChildAt(childIndex - 1).getMeasuredWidth() + 18;
		}
