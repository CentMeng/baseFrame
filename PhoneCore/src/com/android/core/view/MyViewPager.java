...
/***
 * 重写的viewpager继承谷歌提供的控件重载其中方法实现回弹效果
 * 
 * @author mengxc
 * 
 */
public class MyViewPager extends ViewPager {
	...
	public boolean onTouchEvent(MotionEvent arg0) {
		...
		case MotionEvent.ACTION_MOVE:
			...
			if (pagerCount == 1) {
				...
				if (offset > SCROLL_WIDTH) {// 手指滑动的距离大于设定值
					...
					if (getLeft() + (int) (offset * RATIO) >= mRect.left) {
						layout(getLeft() + (int) (offset * RATIO), getTop(),
								getRight() + (int) (offset * RATIO),
								getBottom());
					}
				}
			...
			} else if ((currentItem == 0 || currentItem == pagerCount - 1)) {
				float nowX = arg0.getX();
				float offset = nowX - preX;
				preX = nowX;
				if (currentItem == 0) {
